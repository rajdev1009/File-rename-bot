...
def _print_banner(dev_name: str, dev_tg: str):
    """Print the stylized ASCII deployment banner with proper ANSI colors."""
    CYAN   = "\033[1;36m"
    YELLOW = "\033[1;33m"
    GREEN  = "\033[1;32m"
    RESET  = "\033[0m"
    ...
    banner = (
        ...
        f"{RESET}\n"
        f"{YELLOW}  ┌─────────────────────────────────────────────────────┐\n"
        f"  │  Developer  :  {dev_name:<36}│\n"
        f"  │  Telegram   :  {dev_tg:<36}│\n"
        f"  │  Status     :  {GREEN}Authorized ✅{YELLOW}                              │\n"
        f"  │  Build      :  Production-Grade Asyncio Renamer Bot  │\n"
        f"  └─────────────────────────────────────────────────────┘{RESET}\n"
    )
...
async def _health_server():
    """
    Minimal async HTTP server that answers every GET with 200 OK.

    Koyeb and Render perform TCP/HTTP health checks before marking an
    instance healthy.  Because this bot has no web framework, we spin
    up a raw asyncio TCP server on the PORT env-var (default 8000).
    This coroutine runs concurrently with the bot via asyncio.gather()
    in main() — it never blocks the Telegram event loop.
    """
    port = int(os.environ.get("PORT", "8000"))

    async def _handle(reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
        try:
            await reader.read(1024)      # drain the incoming HTTP request
            writer.write(
                b"HTTP/1.1 200 OK\r\n"
    ...
    log.info("Health-check HTTP server listening on 0.0.0.0:%d", port)
    async with server:
...
async def _run_bot():
    """
    Full Telegram bot lifecycle:
    connect → register handlers → spawn workers → run until disconnected.
    Extracted so it can be run in parallel with _health_server().
    """
    ...
    # ── 8. Spawn persistent worker pool ──────────────────────────
    workers = [
    ...
    log.info(
        "Bot is live. Monitoring channel_id=%d  |  Press Ctrl+C to stop.",
        CHANNEL_ID,
    )

    # ── 9. Run until disconnected / interrupted ───────────────────
    try:
...
async def main():
    ...
    # ── 1. Integrity check — must run first, before anything else ─
    _dev_name, _dev_tg = _run_integrity_check()

    # ── 2. Deployment banner + startup voice message ──────────────
    ...
    # ── 10. Run health server AND bot concurrently ────────────────
    # asyncio.gather() schedules both coroutines on the same event
    # loop.  Neither blocks the other.  Koyeb sees 200 OK on PORT
    # while the bot processes Telegram events simultaneously.
    # return_exceptions=True ensures one crashing does not silently
    # swallow the other's exception.
    try:
